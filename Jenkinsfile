  ...
  stages {
    ...
    stage('Prepare .env') {
      steps {
        dir("${env.MAGENTO_DIR}") {
          script {
            // Create .env from example if present; otherwise generate from params
            sh '''#!/usr/bin/env bash
set -euo pipefail
if [ ! -f .env ]; then
  if [ -f .env.example ]; then
    ...
    echo "[INFO] .env.example not found. Generating .env from pipeline parameters." >&2
    cat > .env <<'ENVEOF'
...
# Custom images (optional overrides)
NGINX_IMAGE=
FPM_IMAGE=
MYSQL_IMAGE=
ELASTICSEARCH_IMAGE=
...
# Container names
NGINX_CONTAINER_NAME=magento-nginx
FPM_CONTAINER_NAME=magento-fpm
MYSQL_CONTAINER_NAME=magento-mysql
ELASTICSEARCH_CONTAINER_NAME=magento-elasticsearch
REDIS_CONTAINER_NAME=magento-redis

# Database settings
MAGENTO_DB_NAME=magento
MAGENTO_DB_USER=magento
MAGENTO_DB_PASSWORD=
MAGENTO_DB_ROOT_PASSWORD=
...
TRAEFIK_STS_INCLUDE_SUBDOMAINS=true
